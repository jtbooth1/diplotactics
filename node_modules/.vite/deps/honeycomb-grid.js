//#region node_modules/honeycomb-grid/dist/honeycomb-grid.mjs
var l = (r) => Number.isFinite(r) && !Number.isNaN(r), p = (r) => typeof r == "object" && r !== null, wt = (r) => p(r) && l(r.q) && l(r.r), V = (r) => typeof r == "function", S = (r) => p(r) && l(r.col) && l(r.row), Z = (r) => p(r) && l(r.x) && l(r.y), H = (r) => Array.isArray(r) && l(r[0]) && l(r[1]), P = (r, t) => t + r * (t & 1) >> 1;
function X(r, t) {
	return (r % t + t) % t;
}
var D = (r, t) => X(r + t, 8), E = ([r, t, e = -r - t]) => ({
	q: r,
	r: t,
	s: e
});
...
}, ct = ({ dimensions: { xRadius: r, yRadius: t }, origin: e, orientation: n }, { x: s, y: o }) => (s += e.x, o += e.y, n === x.POINTY ? L({
...
function ut(r, t) {
	const { q: e, r: n, s } = C(r), { q: o, r: i, s: c } = C(t), u = {
		q: e + o,
		r: n + i,
		s: s + c
	};
...
var ht = [
	...
	if (o === a.S || o === a.N) return z(n, o === a.S ? s + 1 : s - 1, r);
	const i = ht[o];
	return {
		q: t + i.q,
		r: e + i.r
	...
	if (o === a.E || o === a.W) return j(o === a.E ? n + 1 : n - 1, s, r);
	const i = ft[o];
	return {
		q: t + i.q,
		r: e + i.r
...
function dt({ start: r, stop: t }) {
	return function(n, s) {
		const o = [], i = n(r ?? s), h = mt(B(i), B(N(i, t))), f = R(i, i, t), y = 1 / Math.max(f, 1);
		let m = !r && s ? 1 : 0;
		for (; m <= f; m++) {
			const O = L(h(y * m));
			o.push(n(O));
...
function St(r, t) {
	return function(n, s) {
		const { width: o, height: i, start: c, direction: u = a.E } = t ? qt(r, t, n()) : r, h = c ?? s ?? [0, 0], f = _(v({
			start: h,
			direction: D(u, 2),
			length: i
		}), v({
...
function qt(r, t, { isPointy: e, offset: n }) {
	const { col: s, row: o } = K(r, e, n), { col: i, row: c } = K(t, e, n), { swapWidthHeight: y, direction: m } = yt[(s < i ? "A" : "B") + (o < c ? "A" : "B")], O = Math.abs(s - i) + 1, g = Math.abs(o - c) + 1;
	return {
		width: y ? g : O,
		height: y ? O : g,
		start: r,
...
var q = class q {
	...
	static fromJSON({ hexSettings: t, coordinates: e }, n) {
		if (n) {
			const o = e.map(n);
			return new q(o.length > 0 ? o[0].constructor : n({
				q: 0,
				r: 0
			}, 0, [{
				q: 0,
